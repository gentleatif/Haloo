...
router.post('/forgetpassword', async (req, res) => {
  ...
  if (admin) {
    ...
    return res.status(400).send( {error:'No admin with email exist', field: 'email'} );
  }
...
router.post('/setpassword', async (req, res) => {
  ...
  if (!otp || !password || !confirmPassword) {
    return res.status(400).send({ error: 'Otp, password and confirmPassword is required' });
  }

  // check if password and confirmPassword match
  if (password !== confirmPassword) {
    return res.status(400).send({ error: 'Password and confirmPassword does not match', field: 'Password' });
  }
