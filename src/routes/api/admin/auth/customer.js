...
router.post("/generate_otp", async (req, res) => {
  ...
  if (phone.length != 10 || isNaN(phone)) {
    return res
      .status(400)
      .send({ error: "Phone number must be 10 digits", field: "phone" });
  }
...
router.post("/verify_otp", async (req, res) => {
  ...
  if (customer) {
    ...
    if (customer.otp == otp) {
      ...
      // save jws token
      let token = jwt.sign(
        { _id: customer._id, loginType: "user" },
        "config.TOKEN_KEY",
        { expiresIn: "30d" }
      );
      customer.token = token;
      await customer.save();
      // save device token if already not exist
...
router.post("/logout", auth, async (req, res) => {
  ...
  if (customer) {
    ...
    return res
      .status(400)
      .send({ error: "Invalid Customer Id from token please relogin" });
  }
});
